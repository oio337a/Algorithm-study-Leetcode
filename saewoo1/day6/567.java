/* s2가 s1을 연속적으로 포함했는가? 뒤집어도 노상관임 포함 햇냐고 */
...
public class Solution {
	public boolean checkInclusion(String s1, String s2) {
			 ...
			 * 
			 * 덧뺄셈만했는데 니가 얘가 연속적으로 포함했는지 안했는지 어케알아
			 * 문자의 종류를 idx로 구성한게 counter라고 생각함니다 감사합니다
			 */
			 ...
			 * 카운터의 s2 문자(를 인덱스로 사용하는 카운터에서 값)를 빼주면서 기준의 왼쪽 값에는 더해주는 방식으로 검수를 마저 한다
			 * 더하고 빼고 한 뒤에 한바퀴 돌아서 카운터가 모두 0이다? s1을 뒤에서라도 포함했다 true
			 * 끝까지 카운터가 모두 0이되는 케이스가 없다 == s1을 연속적으로 포함하지 않았다 false
			 */
			for (int i = s1.length(); i < s2.length(); i++) {
					count[s2.charAt(i) - 'a']--; // s1 길이보다 우측에서 문자가 발견됨? 빼줘
					count[s2.charAt(i - s1.length()) - 'a']++; // 좌측에서? 더해줘
					if (isZero(count))
							return true;
			}
			return false;
